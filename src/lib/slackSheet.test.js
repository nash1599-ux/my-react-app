...
test("full Friday sheet logs Nate's third G-Unit close and keeps Ismael off the roster", () => {
  const sheet = formatFullSheet(SEED_BOARD);

  expect(sheet.banner).toBe("DG: 18/12 | 37 NL LEFT | FRIDAY");
  expect(sheet.teamApps).toBe(18);
  expect(sheet.teamCx).toBe(10);
